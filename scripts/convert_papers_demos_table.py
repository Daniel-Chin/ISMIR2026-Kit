...
orig_csv = pd.read_csv(
    "../static/csv/ISMIR2020 papers - pre_parse_ISMIR2020_papers.csv"
)
schedule_csv = pd.read_csv(
    "../static/csv/Schedule - Paper (Preliminary) - Schedule - Paper (Preliminary).csv"
)
...
def title_except(s, no_cap_list, unique_cap_list):
    ...
    if word_list[0] in unique_cap_list:  # First always capitalized logic
        ...
        final = (
            [word_list[0][0] + word_list[0][1:].capitalize()]
            if word_list[0][1:] not in unique_cap_list
            else [word_list[0]]
        )
    else:
    ...
    for word in word_list[1:]:
        if word in unique_cap_list:
            ...
            word != "" and word[0] == '"'
        ):  # catch words that should be capitalized after a quote
            (
                final.append(word[0] + word[1:].capitalize())
                if word[1:] not in unique_cap_list
                else final.append(word)
            )
        else:
...
for index, row in orig_csv.iterrows():
    ...
    session_num = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "Session"
    ].values[0]
    slack_channel = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "Slack URL"
    ].values[0]
    ...
    slack_name = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "Slack Channel"
    ].values[0]
    poster_code = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "Poster code"
    ].values[0]
    yt_link = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "Youtube"
    ].values[0]
    bb_link = schedule_csv[schedule_csv["Paper ID"] == row["Paper ID"]][
        "bilibili"
    ].values[0]
    ...
    if key_choice == "m":
        ...
        secondary_sub = (
            [[x.split(" -> ")[1]] for x in row["Secondary Subject Areas"].split("; ")]
            if not pd.isnull(row["Secondary Subject Areas"])
            else None
        )
    else:
        primary_sub = row["Primary Subject Area"].split(" -> ")
        secondary_sub = (
            [x.split(" -> ") for x in row["Secondary Subject Areas"].split("; ")]
            if not pd.isnull(row["Secondary Subject Areas"])
            else None
        )
    secondary_sub = (
        [term for sub in secondary_sub for term in sub] if secondary_sub else None
    )
    ...
    keywords_list.append("|".join(dict.fromkeys(keywords)))
...
new_csv = pd.DataFrame(
    {
        ...
        "title": [
            title_except(x, articles, capital_exceptions)
            for x in orig_csv["Paper Title"]
        ],
...
new_csv_lbds = pd.DataFrame(
    {
        ...
        "title": [
            title_except(x, articles, capital_exceptions)
            for x in orig_csv_lbds["Paper Title"]
        ],
