...
orig_csv = pd.read_csv(
    "../static/csv/ISMIR music program _ Streaming ready submission (Responses) - Form Responses 1 for site.csv"
)
...
new_csv = pd.DataFrame(
    {
        ...
        "title": [
            (
                "".join((title, ": The Voice of Santur with Audience Participation"))
                if "Sunrise" in title
                else title
            )
            for title in orig_csv["Title of the piece"]
        ],
        "abstract": orig_csv[
            "Abstract of the piece. This abstract is going to be displayed on the ISMIR music program page. (150-250 words)"
        ],
        ...
        "yt_id": [
            link.split("/")[-1] if isinstance(link, str) else link
            for link in orig_csv["YouTube link"]
        ],
        "bb_id": [
            link.split("/")[-1] if isinstance(link, str) else link
            for link in orig_csv["Bilibili link"]
        ],
        "db_link": orig_csv["Dropbox link"],
        "authors": orig_csv["Authors"],
        "still_image": orig_csv["Still image (for audio-only submissions). "],
        "release_consent": [
            1 if isinstance(x, str) else 0 for x in orig_csv["Consent"]
        ],
    }
)
