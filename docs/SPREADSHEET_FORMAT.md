...
- Google Drive media: authors give `https://drive.google.com/open?id=...` share
  links (the `raw_*` columns). The site converts these to `/file/d/<id>/preview`
  at render time.
...
- ⚙️ = written back by `miniconf_prep.py` / `utils/slack.py` / `utils/zoom.py`.
  ...
  `python pull_from_google_sheet.py` run or they will be overwritten.
...
abstract, paper_presentation, primary_subject,
...
```bash
export PAPERS_SHEET_URL="https://docs.google.com/spreadsheets/d/<sheet-id>/edit?gid=<tab-gid>#gid=<tab-gid>"
export EVENTS_SHEET_URL=..
export LBDS_SHEET_URL=...
export MUSIC_SHEET_URL=...
export INDUSTRY_SHEET_URL=...
export SESSION_ASSIGNMENT_SHEET_URL=...
python pull_from_google_sheet.py            # sheet tabs → sitedata/*.csv
